...
const API_URL = "http://192.168.235.182:5001/api/incidents/active";
...
// Helper to extract incidents from the API response
const extractIncidents = (data: IncidentDataResponse): Incident[] => {
...
export default function TechCrisis() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const result: IncidentDataResponse = await response.json();

      if (result?.success && result?.data?.incidents) {
        setIncidents(extractIncidents(result));
  ...
  const avgConfidence = useMemo(() => {
    ...
    return Math.round(
      withAnalysis.reduce(
        (acc, i) => acc + (i.analysis?.confidence_score || 0),
        0
      ) / withAnalysis.length
    );
  }, [incidents]);
      ...
      >
        <div
          className={cn(
            ...
            config.bg,
            config.border,
            incident.severity === "P0" && "glow-critical"
          )}
        >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mt-5">
                  ...
                  <div className="group rounded-xl border border-status-critical/20 bg-gradient-to-br from-status-critical/5 to-transparent p-4 transition-all hover:border-status-critical/40 hover:shadow-lg hover:shadow-status-critical/5">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <h4 className="font-semibold text-status-critical">
                        Root Cause
                      </h4>
                    </div>
                  ...
                  <div className="group rounded-xl border border-status-success/20 bg-gradient-to-br from-status-success/5 to-transparent p-4 transition-all hover:border-status-success/40 hover:shadow-lg hover:shadow-status-success/5">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <h4 className="font-semibold text-status-success">
                        Remediation Steps
                      </h4>
                    </div>
                    <div className="text-sm text-foreground/80 leading-relaxed space-y-2">
                ...
                <div className="bg-muted/40 backdrop-blur-sm border border-border/50 p-4 rounded-xl text-xs font-mono overflow-x-auto">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-foreground/90 truncate">
                        {incident.raw_data.applicationName}
                      </p>
                    </div>
                    <div className="space-y-1">
                      <span className="text-muted-foreground/70 uppercase tracking-wider text-[10px]">
    ...
    <div className="space-y-6 animate-slide-in">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Tabs
            value={viewMode}
            onValueChange={(v) => setViewMode(v as "tech" | "non-tech")}
          >
            <TabsList>
      ...
      {viewMode === "non-tech" ? (
        <NonTechCrisisView incidents={incidents} />
      ) : (
        <>
          {/* Summary Stats */}
            ...
            <Card className="shadow-md border-l-4 border-l-status-critical">
              <CardContent className="pt-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-3xl font-bold font-heading text-status-critical">
                      {p0Count}
                    </p>
            ...
            <Card className="shadow-md border-l-4 border-l-status-warning">
              <CardContent className="pt-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-3xl font-bold font-heading text-status-warning">
                      {p1Count}
                    </p>
                    ...
                    return (
                      ...
                      >
                        <span
                          className={cn(
                            "text-sm truncate",
                            hasP0 && "text-status-critical font-medium"
                          )}
                        >
                          {service}
                        </span>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-muted-foreground">
                            {items.reduce((acc, i) => acc + i.alert_count, 0)}{" "}
                            alerts
                          </span>
                        </div>
                      </div>
                ...
                {[
                  {
                    ...
                    alerts: incidents
                      ...
                      .reduce((acc, i) => acc + i.alert_count, 0),
                    color: "text-status-critical",
                  },
                  {
                    severity: "P1",
                  ...
                  {
                    severity: "P3",
                    count: incidents.filter((i) => i.severity === "P3").length,
                    alerts: incidents
                  ...
                  .map(({ severity, count, alerts, color }) => (
                    <div
                      key={severity}
                      className="flex items-center justify-between"
                    >
